...
namespace QuanLyNhanSuAPI.Controllers.HoSoNhanSuController
{
    ...
    public class ThongTinNhanVienController : ControllerBase
    {
        ...
        [HttpPost("kiemtraemail/{email}")]
        public async Task<ActionResult<ServiceResponse<bool>>> KiemTraEmail(string email)
        {
            var result = await _nhanvienService.IsEmailUnique(email);
            return Ok(result);
        }
