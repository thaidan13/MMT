...
namespace QuanLyNhanSuWEB.Controllers
{
    ...
    public class DaoTaoCuMoiController : Controller
    {
        private readonly HttpClient _http;

        public DaoTaoCuMoiController(HttpClient http)
        {
            _http = http;
        }
        ...
        public async Task<ActionResult<ServiceResponse<DaoTaoCuMoiDTO>>> Index(int nhanvienId)
        {
            ...
            var daotao = await _http.GetFromJsonAsync<ServiceResponse<List<TbQuaTrinhDaoTaoCuMoi>>>($"http://10.0.0.4:5259/api/QuaTrinhDaoTaoCuMoi/daotaocu/{nhanvienId}");
